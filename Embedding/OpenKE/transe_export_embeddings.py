...
def main():
    checkpoint_path = "./checkpoint/transe/transe_2026-01-05_15-26-59.ckpt" # checkpoint path
    ...
    entity_embeddings = transe.ent_embeddings.weight
    entity_embeddings = entity_embeddings.cpu().detach().numpy()
    with open(f'./embeddings/transe_entity_embeddings_{current_time}.txt', 'w') as f:
    ...
    relation_embeddings = transe.rel_embeddings.weight
    relation_embeddings = relation_embeddings.cpu().detach().numpy()
    with open(f'./embeddings/transe_relation_embeddings_{current_time}.txt', 'w') as f:
