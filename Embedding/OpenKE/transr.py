...
def main():
    ...
    os.makedirs("./checkpoint/transr", exist_ok=True)
    os.makedirs("./reports", exist_ok=True)

    checkpoint_path = f'./checkpoint/transr/transr_{current_time}.ckpt'
    report_path = f'./reports/transr_cost_report_{current_time}.json'

    # ===== Hyperparameter (TransR) =====
    nbatches = 200
    neg_samples = 20
    embedding_dim = 50
    alpha_value = 0.5
    ...
    # ===== Save checkpoint =====
    t0 = time.perf_counter()
    transr.save_checkpoint(checkpoint_path)
    t1 = time.perf_counter()
